...
char* b64encode(char* input, int length)
{
	...
	int b64size = b64e_size(length) + 1;
	char* encodedstr = (char*)malloc(b64size);
	memset(encodedstr, 0, b64size);
	b64_encode((unsigned char*)input, length, (unsigned char*)encodedstr);
	return encodedstr;
}

char* b64decode(char* input, int length)
{
	int b64size = b64d_size(length) + 1;
	char* decodedstr = (char*)malloc(b64size);
	memset(decodedstr, 0, b64size);
	b64_decode((unsigned char*)input, length, (unsigned char*)decodedstr);
	return decodedstr;
}
